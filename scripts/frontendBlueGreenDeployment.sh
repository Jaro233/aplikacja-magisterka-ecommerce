...
if [ "$DEPLOY_ONLY" == "true" ]; then
  ...
  helm upgrade "${APP_NAME_SHORT}" ./kubernetes/helm/app/"${APP_NAME_SHORT}" --set microservice.profile_env_${newSlot}=kubernetes --set microservice.current_environment_${newSlot}=prod --set microservice.image.repository_${newSlot}=${ECR_REGISTRY}/${ECR_REPO_NAME} --set microservice.image.tag_${newSlot}=1.${REPOSITORY_TAG} --reuse-values
  ...
  helm upgrade "${APP_NAME_SHORT}" ./kubernetes/helm/app/"${APP_NAME_SHORT}" --set microservice.profile_env_${oldSlot}=kubernetes-stage --set microservice.current_environment_${oldSlot}=stage --set microservice.image.repository_${oldSlot}=${ECR_REGISTRY}/${ECR_REPO_NAME} --set microservice.image.tag_${oldSlot}=1.${REPOSITORY_TAG} --reuse-values
